...
def home_view(request):
    """
    Add the users recipes to the cookbook, created and saved. the three latest
    recipes is displayed as a feature.
    """
    ...
    if request.user.is_authenticated:
        ...
        if letter_filter:
            cookbook_entries = cookbook_entries.filter(
                recipe__title__istartswith=letter_filter
            )

        if sort_order == 'likes':
            cookbook_entries = cookbook_entries.annotate(num_likes=Count(
                'recipe__likes')).order_by('-num_likes')
        else:
        ...
        for entry in cookbook_entries:
            recipe = entry.recipe
            if recipe.photo:
                recipe.photo_url, options = cloudinary_url(
                    recipe.photo.public_id, width=400, height=300, crop="fill"
                )
            else:
